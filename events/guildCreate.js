...
client.on("guildCreate", async (guild, Client) => {
    ...
    const ch = guild.channels.cache.find(channel => channel.type === ChannelType.GuildText && channel.permissionsFor(guild.me).has(PermissionFlagsBits.SendMessages) && channel.permissionsFor(guild.me).has(PermissionFlagsBits.EmbedLinks));


    if (!ch) return;

    if (!guild.me.permissions.has(PermissionFlagsBits.Administrator)) {
        try {
            ...
            if (ch && ch.permissionsFor(guild.me).has(PermissionFlagsBits.SendMessages) && ch.permissionsFor(guild.me).has(PermissionFlagsBits.EmbedLinks) && ch.permissionsFor(guild.me).has(PermissionFlagsBits.ViewChannel)) {
                await ch.send({
                    embeds: [
                        new EmbedBuilder()
                        .setColor(ee.wrongcolor)
