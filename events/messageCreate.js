...
client.on("messageCreate", async (message) => {
    ...
    const finduser = await findUser(message.author.id);

    if (finduser) {
        await giveActivityXP(message, xpCooldowns);
    }

    const findserver = await findServer(message.guild.id);
    ...
    if (!findserver) {
        ...
        await increaseSpawnChance(findserver, finduser, awardCooldowns, message);
    }
    ...
    try {
        ...
        if (spawntime >= 50) {
            ...
            if (message.channel.permissionsFor(message.guild.members.me).has(PermissionFlagsBits.SendMessages) && message.channel.permissionsFor(message.guild.members.me).has(PermissionFlagsBits.EmbedLinks) && message.channel.permissionsFor(message.guild.members.me).has(PermissionFlagsBits.ViewChannel)) {
                await encounterspawn(message, spawnedrarity);
            } else {
                return;
            }
        }
